...
```
SELECT name, MAX(Years_employed) AS longest_employee
FROM employees;
```
...
```
SELECT building, SUM(years_employed) AS Sum_of_years
FROM employees
GROUP BY Building;
```
...
```
SELECT role, COUNT(name) AS Employees
FROM employees
GROUP BY role;
```
...
```
SELECT director, SUM(Domestic_sales + International_sales) AS Overall_Sales
FROM movies 
INNER JOIN Boxoffice
...
1. Weve sadly reached the end of our lessons, lets clean up by removing the Movies table
```
